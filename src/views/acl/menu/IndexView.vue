<template>
    <el-table style="width: 100%; margin-bottom: 20px" :data="PermissionArr" row-key="id" border>
...
import { MenuParams, Permission, PermissionResponseData, reqAddOrUpdateMenu, reqAllPermission, reqRemoveMenu } from '@/api/acl/menu';
import { ElMessage } from 'element-plus';
import { ref, onMounted, reactive } from 'vue'

const PermissionArr = ref<Permission[]>([])
const dialogVisible = ref<boolean>(false)
const menuData = reactive<MenuParams>({ code: '', level: 0, name: '', pid: 0})

const getHasPermission = async () => {
    const res: PermissionResponseData = await reqAllPermission()
    if (res.code === 200)   PermissionArr.value = res.data
}
